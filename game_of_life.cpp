//
//  game_of_life.cpp
//  Game Of Life Pointers - parameter is an array containing pointers
//
...
int main(int argc, const char * argv[]) {
    ...
    // Create the grid
    bool *theGrid[GRID_SIZE];
    bool *tmpGrid[GRID_SIZE];
    
    for (int i = 0; i < GRID_SIZE; i++)
    {
        theGrid[i] = new bool[GRID_SIZE];
        tmpGrid[i] = new bool[GRID_SIZE];
    }
    
    
    zeroGrid(theGrid);                                              // Whole grid zero'd
    zeroGrid(tmpGrid);                                              // Whole grid zero'd
